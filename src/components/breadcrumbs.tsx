const BreadCrumbs = ({ navs }: { navs: any}) => {
    return (
        <nav className="outer-container !py-3 capitalize" aria-label="Breadcrumb">
            <div className="md:flex items-center space-x-1 md:space-x-2  border px-5 py-2 border-gray-300">
                {navs.map((item: any, index: number) => (
                    <div key={index} className="flex items-center">
                        ...
                        <a href={item.href} className={`inline-flex items-center text-sm font-medium ${index === navs.length - 1 ? 'text-gray-500 pointer-events-none' : 'text-gray-700 hover:text-primary'}`}>
                            {item.name}
                        </a>
                    </div>
                ))}
