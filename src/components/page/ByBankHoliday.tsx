...
export default function ByBankHoliday() {
    const { Get } = useApiRequest();
    const [currentPage,] = useState<number>(1);
    const [queryParams,] = useState<string>('');

    const { data: listings } = Get(`/car-boot?page=${currentPage}&filter_by=${'bank_holiday'}${queryParams}`) as { data: { items: ListingType[] }, loading: boolean };

    return (
        <main className="bg-[#f3f7fe]">
            <BreadCrumbs navs={[{ name: 'Home', href: '/' }, { name: 'Bank Holiday Car Boot Sales' }]} />
            <div className='outer-container'>
                <h1 className='text-2xl font-bold capitalize'>Bank Holiday Car Boot Sales</h1>
                <p className='text-gray-600'>Discover Bank Holiday Car Boot Sales top-rated Car Boot Sales across the UK. View listings, schedules, and visitor ratings.</p>
                <div className='mt-5'>
