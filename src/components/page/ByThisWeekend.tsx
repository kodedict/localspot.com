...
export default function ByThisWeekend() {
    const { Get } = useApiRequest();
    const [currentPage,] = useState<number>(1);
    const [queryParams,] = useState<string>('');

    const { data: listings } = Get(`/car-boot?page=${currentPage}&filter_by=${'this_weekend'}${queryParams}`) as { data: { items: ListingType[] }, loading: boolean };

    return (
        <main className="bg-[#f3f7fe]">
            <BreadCrumbs navs={[{ name: 'Home', href: '/' }, { name: 'Car Boot Sales this weekend ' }]} />
            <div className='outer-container'>
                <h1 className='text-2xl font-bold capitalize'>Car Boot Sales this weekend </h1>
                <p className='text-gray-600'>Discover top-rated Car Boot Sales this weekend across the UK. View listings, schedules, and visitor ratings.</p>
                <div className='mt-5'>
