...
const RelatedListing = () => {
    ...
    useEffect(() => {
        ...
        if (coordinatesStr) {
            setUserCoordinates(JSON.parse(coordinatesStr));
        }
    }, []);
    ...
    const { data: listings } = Get(`/car-boot?page=${currentPage}${queryParams}&per_page=3&latitude=${latitude}&longitude=${longitude}`) as { data: { items: ListingType[] }, loading: boolean };
                        ...
                        <Link key={index} href={`/${(item.category === 'nil' || !item.category) ? 'car-boot-sales' : item.category}/${item.region || 'london'}/${item.code}/${item.slug}`} className="flex flex-col" style={{ boxShadow: "rgba(17, 12, 46, 0.15) 0px 48px 100px 0px" }}>
                            <div className="md:p-6 p-4">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <div className="inline-flex items-center font-medium hover:text-primary-dark gap-4 px-5 border py-2 hover:bg-primary-light hover:bg-opacity-10 rounded border-gray-400">
